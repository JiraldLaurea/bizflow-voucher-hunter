...
// Dev-tools helper behind the More page's dev tools: clears one phone's hunt
// (attempts, voucher, reservation) across every campaign it has hunted and
...
//
// The gate runs after authentication rather than before it, because the
// production developer account is allowed through and it is the session that
// identifies it. Nothing leaks by ordering it this way: the reset only ever
...
export async function POST(request: Request) {
  try {
    ...
    assertDevToolsEnabledFor(phone, "Hunt reset");
